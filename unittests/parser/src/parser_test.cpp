...
TEST(ParserTest, ParsesBasicFunctionStructure) {
    ...
    const Program& program = *parse_result.program;
    expect_kind(program, NodeKind::Program);
    ASSERT_EQ(program.children_nodes.size(), 1);

    const auto* function_def =
        static_cast<const FunctionDef*>(program.children_nodes[0].get());
    ...
    expect_kind(child(parameter_list, 0), NodeKind::Parameter);

    const Node& compound_stmt = child(*function_def, 1);
    ASSERT_EQ(compound_stmt.children_nodes.size(), 6);
    expect_kind(child(compound_stmt, 0), NodeKind::ExpressionStmt);
    expect_kind(child(compound_stmt, 1), NodeKind::ExpressionStmt);
    expect_kind(child(compound_stmt, 2), NodeKind::ForStmt);
    expect_kind(child(compound_stmt, 3), NodeKind::IfStmt);
    expect_kind(child(compound_stmt, 4), NodeKind::ExpressionStmt);
...
TEST(ParserTest, ParsesForAndIfElseShape) {
    ...
    expect_kind(child(*for_stmt, 1), NodeKind::BinaryOperExpr);
    ...
    expect_kind(child(if_stmt, 0), NodeKind::BinaryOperExpr);
...
TEST(ParserTest, ParsesBinaryPrecedenceCorrectly) {
    ...
    expect_kind(expr, NodeKind::BinaryOperExpr);
    EXPECT_EQ(expr.node_name, "AdditiveExpr");

    const Node& left = child(expr, 0);
    const Node& right = child(expr, 1);

    expect_kind(left, NodeKind::IntegerExpr);
    expect_kind(right, NodeKind::BinaryOperExpr);
